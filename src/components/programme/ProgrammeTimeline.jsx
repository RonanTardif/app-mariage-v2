      ...
      {groups.map(({ day, events: dayEvents }) => (
        <div key={day}>
          ...
          <div className="relative">
            {/* Ligne verticale continue — centrée sur la colonne dot (52px + 10px) */}
            <div className="absolute left-[62px] top-3 bottom-3 w-px bg-stone-200" />
...
function TimelineItem({ event, idx, isPast, isCurrent, isNext }) {
  return (
    ...
    >
      {/* Colonne heure : 52px, alignée à droite */}
      <div className="flex w-[52px] shrink-0 items-start justify-end pr-2 pt-3.5">
        <span
      ...
      {/* Colonne dot : 20px, dot centré sur la ligne */}
      <div className="relative z-10 flex w-5 shrink-0 items-start justify-center pt-3">
        {isCurrent ? (
          ...
          /* Check pour passé */
          <span className="flex h-3 w-3 items-center justify-center rounded-full bg-stone-200">
            <Check size={7} className="text-stone-400" strokeWidth={3} />
          </span>
        ) : isNext ? (
          /* Cercle plein pour prochain */
          <span className="h-3 w-3 rounded-full bg-stone-600 ring-2 ring-white" />
        ) : (
          /* Cercle vide pour futur */
          <span className="h-3 w-3 rounded-full border-2 border-stone-200 bg-white" />
        )}
      </div>
